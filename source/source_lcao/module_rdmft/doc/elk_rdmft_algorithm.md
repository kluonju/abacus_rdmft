...
**Kohn–Sham occupations inside `occupy`:** ELK finds `efermi` by bisection so the k-weighted sum of occupations equals the target valence charge `chgval`. Only states in a **narrow energy window** around $e_{\mathrm{F}}$ pick up **fractional** occupations from smearing; deep occupied and high empty states are (essentially) 1 or 0. When mapping this idea to ABACUS RDMFT’s `rdmft_occ_init_mode = perturbed`, the code applies the `±\delta` shift **only to a Fermi window** of bands (set by `rdmft_occ_init_nbands_top`, with a small default window when that keyword is unset), rather than to every band index at each **k**—consistent with not perturbing the whole valence manifold. Formally, for each band $i$ and k-point $k$,
