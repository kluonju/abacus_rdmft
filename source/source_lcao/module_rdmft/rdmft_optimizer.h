...
class BarzilaiBorweinStep
{
  public:
                   ...
                   const double fallback_alpha)
    {
        ...
        {
            // No BB pair (s, y) yet: use a spectral-style first trial
            //   α ≈ ||x|| / ||g||  (same units as BB1/BB2 in Euclidean form),
            // falling back to `fallback_alpha` when the ratio is unusable.
            return clamp_alpha(cold_start_alpha(x, grad, fallback_alpha));
        }
  ...
  private:
    static double cold_start_alpha(const std::vector<double>& x,
                                   const std::vector<double>& grad,
                                   const double fallback_alpha)
    {
        double xn2 = 0.0;
        ...
        const double xn = std::sqrt(xn2);
        const double gn = std::sqrt(gn2);
        constexpr double eps = 1.0e-30;
        if (!(gn > eps) || !std::isfinite(gn) || !std::isfinite(xn))
        {
            return fallback_alpha;
        }
        const double a = xn / gn;
...
/// Strong Wolfe with a **non-monotone** sufficient decrease:
/// \f$f(\alpha)\le f_{\mathrm{ref}}+c_1\alpha\phi'(0)\f$ (Zhang–Hager-style
/// reference \f$f_{\mathrm{ref}}\f$ fixed for this search). Curvature tests
/// match `strong_wolfe_line_search` (use \f$\phi'(0)=g_0\f$, trial slopes
/// \f$g(\alpha)\f$ from `phi`).
inline LineSearchResult strong_wolfe_nm_line_search(
    std::function<std::pair<double, double>(double)> phi,
    double f0,
    double g0,
    double f_ref,
    ...
    int max_zoom = 20)
{
    ...
    auto zoom = [&](double alpha_lo, double f_lo, double g_lo, double alpha_hi, double f_hi, double g_hi) -> LineSearchResult
    {
        ...
        for (int j = 0; j < max_zoom; ++j)
        {
            ...
            if (f_j > f_ref + c1 * alpha_j * g0 || f_j >= f_lo)
            {
                alpha_hi = alpha_j;
                f_hi = f_j;
                g_hi = g_j;
    ...
    for (int i = 0; i < max_iter; ++i)
    {
        ...
        if (f_i > f_ref + c1 * alpha * g0 || (i > 0 && f_i >= f_prev))
        {
            LineSearchResult zr
                = zoom(alpha_prev, f_prev, g_prev, alpha, f_i, g_i);
            return zr;
...
/// Weak Wolfe line search (Nocedal & Wright, Algorithm 3.5 + 3.6, weak curvature).
///
/// `phi(alpha)` returns `{ f(alpha), g(alpha) }` where `g(alpha)` is the
/// directional derivative along the search line. For minimization with descent
/// direction, require `g0 < 0`.
///
/// Weak Wolfe conditions:
///   1) f(alpha) <= f0 + c1 * alpha * g0
///   2) g(alpha) >= c2 * g0
inline LineSearchResult weak_wolfe_line_search(
    std::function<std::pair<double, double>(double)> phi,
    double f0,
    double g0,
    double alpha_init = 1.0,
    ...
    int max_zoom = 20)
{
    ...
    auto zoom = [&](double alpha_lo, double f_lo, double g_lo, double alpha_hi, double f_hi, double g_hi) -> LineSearchResult
    {
        ...
        for (int j = 0; j < max_zoom; ++j)
        {
            ...
            std::pair<double, double> fg_j = phi_wrap(alpha_j);
            const double f_j = fg_j.first;
            const double g_j = fg_j.second;
    ...
    {
        std::pair<double, double> fg_i = phi_wrap(alpha);
        const double f_i = fg_i.first;
        const double g_i = fg_i.second;

        if (f_i > f0 + c1 * alpha * g0 || (i > 0 && f_i >= f_prev))
        {
            return zoom(alpha_prev, f_prev, g_prev, alpha, f_i, g_i);
        }
