...
bool not_supported_xc_with_nonlocal_vdw(const std::string& xc_func_in)
{
    ...
    {
        if(xc_func.find(str) != std::string::npos)
        {
            return true;
        }
    }
...
XC_Functional_Libxc::set_xc_type_libxc(const std::string& xc_func_in)
{
    ...
    std::transform(xcfunc_words_.begin(),
                   xcfunc_words_.end(),
                   xcfunc_type_.begin(),
                   [](const std::string& func)
                   {
                       return xc_func_type_classifier(func);
                   });
    if (std::adjacent_find(xcfunc_type_.begin(),
                           xcfunc_type_.end(),
                           [](int a, int b)
                           {
                               return a != b;
    ...
    std::transform(xcfunc_words_.begin(),
                   ...
                   [](const std::string& func)
                   {
                       return xc_functional_get_number(func.c_str());
                   });
    // if there is any -1, it means the functional is not recognized
    const bool not_recognized_xc = std::any_of(func_id.begin(),
                                               func_id.end(),
                                               [](int id)
                                               {
                                                   return id == -1;
