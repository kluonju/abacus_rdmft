...
//    bool final_scf = false;    ///< whether to do final scf
    ...
    bool rdmft_orb_precond = true;              // orbital CG/L-BFGS rotation preconditioner switch: on/off
    /// Orbital CG level-shift delta in 1/(|e_i-e_j|+delta); <=0 uses internal auto scale.
    double rdmft_orb_precond_delta = 0.0;
    ...
    /// Log one-body / Hartree / EXX orbital gradient norms each RDMFT energy+grad compute.
    bool rdmft_print_orb_grad_decomp = false;
