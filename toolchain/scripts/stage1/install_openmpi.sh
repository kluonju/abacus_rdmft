...
# Load OpenMPI package variables with version suffix support
...
if [[ -n "${ABACUS_TOOLCHAIN_PACKAGE_VERSIONS}" ]]; then
    ...
    if echo "${ABACUS_TOOLCHAIN_PACKAGE_VERSIONS}" | grep -q "openmpi:alt"; then
        version_suffix="alt"
    elif echo "${ABACUS_TOOLCHAIN_PACKAGE_VERSIONS}" | grep -q "openmpi:main"; then
        version_suffix="main"
    fi
fi
...
load_package_vars "openmpi" "$version_suffix"
...
case "${with_openmpi}" in
    ...
    *)
        ...
        check_command ${pkg_install_dir}/bin/mpicxx "openmpi" && MPICXX="${pkg_install_dir}/bin/mpicxx" || exit 1
...
if [ "${PACK_RUN}" = "__TRUE__" ]; then
    echo "--pack-run mode specified, skip env setting"
else
